...
with tf.device(device_name):
    m1 = tf.random_uniform(shape=shape, minval=0, maxval=1)
    m2 = tf.random_uniform(shape=shape, minval=0, maxval=1)
...
with tf.Session(config=config) as sess:
    ...
    trace_file = tf.gfile.Open(name='timeline2.json', mode='w')
